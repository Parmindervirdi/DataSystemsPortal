...
<?
   ...
   $develpername=$row_customers[12]." ".$row_customers[13];
   $hastransfers=$row_customers[25];
   $hasPSAs=$row_customers[26];
   $hasScheudle=$row_customers[27];
	$hasbustime=$row_customers[28];
	$hasCAD=$row_customers[29];
	$hasFareBox=$row_customers[30];
	$hasCanneddate=$row_customers[31];
	$hasgeofences=$row_customers[32];
	$hasTSPs=$row_customers[33];
	$hasTCPs=$row_customers[34];
	$hasPrePosttrips=$row_customers[35];
	$hasheadways=$row_customers[36];
	$hasmanualrotues=$row_customers[37];
	$hasLBAs=$row_customers[38];
	$hasstoprequested=$row_customers[39];
	$hasotherlanguage=$row_customers[40];
	$hasTurnWarning=$row_customers[41];
	$numberofroutes=$row_customers[42];
	$numberoftestrotues=$row_customers[43];
	$numberofstops=$row_customers[44];
	$numberofdepots=$row_customers[45];
	$numberofhubs=$row_customers[46];
	$destint=$row_customers[47];
	$Globalsdocs=$row_customers[48];
	$multileveldest=$row_customers[49];
	$texttospeech=$row_customers[53];
	$cw=$row_customers[54];
	$bw=$row_customers[55];
