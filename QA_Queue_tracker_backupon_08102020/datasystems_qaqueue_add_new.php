...
if (isset ($_POST["controlflow"]))
{
	...
	$schedule_enddate=$_POST["Schend"];
	$schedule_startdate=$_POST["Schstart"];
	
	if ($schedule_enddate=='')
	{
	   $schedule_enddate='0000-00-00';
	}
	
	
	if ($schedule_startdate=='')
	{
	   $schedule_startdate='0000-00-00';
	}
	...
	$insert_qaqueue="insert into DBS_QAQUEUE (Customer,QA_Startdata,Actination_date,QA_Priority,DB_releasedate,Comments,DB_Status,FixedVersion,QA_Ticket,Pick,CreatedBy,CreatedDate,CleverWorksVersion,Scheduleenddate,Schedulestartdate)
                                      values ($customer,'$qasdate','$actdate',$dbpror,'$reldate','$comm','$stat','$FixedVersion','$QA_ticket',$despick,'$username','$create_date','$cleverworksversion','$schedule_enddate','$schedule_startdate')";	
	...
	$update_cb="update dbs_customer_base set CWVersion='$cleverworksversion',Scheduleenddate='$schedule_enddate',Schedulestartdate='$schedule_startdate' where CustomerID=$customer";
	mysqli_query($conn_jira_local,$update_cb);
	###Update Epic with Schedule Start and End date 
	shell_exec("python ../../../Python_Code/update_Epic_schend_schstart.py  $FixedVersion  $schedule_startdate  $schedule_enddate");
...
function addvalue_Cleverworks(e) 
	...
	CW_version=cust[1]
	schedenddate=cust[2]
	schedstartdate=cust[3]
	document.getElementById("getdata").value = CW_version
	document.getElementById("getenddate").value = schedenddate
	document.getElementById("getstartdate").value = schedstartdate
}		   
</script>	
</head>
<body>
					...
					<tr>
					      <table width=400 align="center">
							   ...
							   <tr> 
								   ...
								   <td class="record_odd">
								                   <select name="Customer" onchange="addvalue_Cleverworks(event)">
														 ...
														 { ?>
															   <option value=<?echo $row_fetch_customer[0].':'.$row_fetch_customer[3].':'.$row_fetch_customer[10].':'.$row_fetch_customer[12];;?>><?echo $row_fetch_customer[1];?></option>
							   ...
							   <tr> 
								   ...
								   <td class="record_odd"><input id="getstartdate" type="date" name="Schstart" ></td>
							   </tr>
							   <tr> 
							       <td class="record_odd">Schedule End Date</td>
								   <td class="record_odd"><input id="getenddate" type="date" name="Schend" ></td>
							   </tr>
							    <tr> 
