...
if (isset ($_POST["controlflow"]))
{
	...
	$exportreason=$_POST['reasonofexport'];
	
	if ($testtype=="fulltest")
	{
		$testemailtype="Full Regression";
	}
	else if ($testtype=="Loadtest")
	{
		$testemailtype="Load Test Only";
	}
	
	##Select the Name and ID of customer 
	$dev_tl_name=whoisdev($customer,$conn_jira_local);
	...
	$insert_exportdata="Insert into exportcreated (DatabaseID,CustomerID,ExportNumber,TeamcityLink,DateAdded,Addedby,DevNote,TestType,ExportReason) values ($qid,'$customer','$exportnumber','$teamcitylink',CURDATE(),'$username','$devnote','$testtype','$exportreason')";
	...
	shell_exec("py ../../../Python_Code/Addcomments.py $qaticket $exportnumber $devnote_py $teamcitylink_py");
					...
					<tr>
					      <table width=400 align="center">
							   ...
							   <tr> 
							     ...
							     <tr> 
							       <td class="record_even">Export Reason</td>
								   <td class="record_even"><select name="reasonofexport">
								                                  <option value='0'>select Reason</option>
															 <? $select_reason="select * from dbs_exportreason order by reasonID";
															    $execute_reason=mysqli_query($conn_jira_local,$select_reason);
																echo $select_reason;
																while ($row_reason=mysqli_fetch_array($execute_reason))
																{?>
								                                   <option value=<?echo $row_reason[1];?>><?echo $row_reason[2];?></option>
															  <?}?>																
								                          </select>
								   </td>
							   </tr>
							   ...
							   <tr> 
							       <td class="record_odd">Type of Testing</td>
								   <td class="record_odd"><select name="testtype">
								   <Option value="na">Select Type</option>
								   <Option value="fulltest">Full Regression</option>
								   <Option value="Loadtest">Load Test Only</option>
								   </select>
								   </td>
							   </tr>
							   <tr> 
